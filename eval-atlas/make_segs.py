...
class ploras:
    def __init__(
        ...
        args = SimpleNamespace(
            exec_mode="predict",
            data="/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/data/16_3d/test",
            results="/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/results",
            config="../docker-atlas/config/config.pkl",
            logname="ploras",
            task="16",
            gpus=1,
            nodes=1,
            ...
            val_epochs=10,
            ckpt_path=None,
            ckpt_store_dir="../docker-atlas/checkpoints/",
            fold=0,
            patience=100,
            batch_size=4,
            val_batch_size=4,
        ...
        for i, pth in enumerate(self.model_paths):
            ...
            ckpt["hyper_parameters"][
                "args"
            ].ckpt_store_dir = "../docker-atlas/checkpoints/" + str(i)
            ckpt["hyper_parameters"]["args"].ckpt_path = (
                "../docker-atlas/checkpoints/" + str(i) + "/best.ckpt"
            )
            ckpt["hyper_parameters"]["args"].fold = i
            ckpt["hyper_parameters"]["args"].gpus = (
    ...
    def crf(self, image, pred):
        ...
        pair_energy = create_pairwise_bilateral(
            sdims=(5.0,) * 3, schan=(5.0,) * image.shape[-1], img=image, chdim=3
        )
    ...
    def nnunet_preprocess(self, image):
        os.makedirs(
            "/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/data/ATLAS2022_ss/imagesTs/",
            exist_ok=True,
        )
        ...
        with open(
            "/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/data/ATLAS2022_ss/dataset.json",
            "w",
        ) as f:
            json.dump(data_desc, f)
        args = SimpleNamespace(
            data="/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/data",
            results="/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/data",
            exec_mode="test",
            ohe=False,
            verbose=False,
            task="16",
    ...
    def nnunet_infer(self, args):
        ...
        save_dir = os.path.join(
            "/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/prediction",
            str(args.fold),
        )
    ...
    def setup(self):
        os.makedirs(
            "/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/data", exist_ok=True
        )
        os.makedirs(
            "/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/results", exist_ok=True
        )
        os.makedirs(
            "/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/prediction",
            exist_ok=True,
    ...
    def predict(self, input_data):
        ...
        paths = [
            os.path.join(
                "/home/lchalcroft/mdunet/miccai22_dockers/eval-atlas/prediction",
                str(i),
                "ATLAS2022_ss_0001.npy",
            )
            for i in range(len(self.args))
        ]
        prediction = self.nnunet_ensemble(
            paths, ref=t1w_image if self.preprocessed else t1w_image_n4ss
        )

        pred_crf = SimpleITK.GetArrayFromImage(prediction)
        pred_crf = np.stack([1.0 - pred_crf, pred_crf])
        img_crf = SimpleITK.GetArrayFromImage(
        ...
        if self.preprocessed:
            prediction.SetOrigin(t1w_image.GetOrigin()), prediction.SetSpacing(
                t1w_image.GetSpacing()
            ), prediction.SetDirection(t1w_image.GetDirection())
        else:
            prediction.SetOrigin(t1w_image_n4ss.GetOrigin()), prediction.SetSpacing(
                t1w_image_n4ss.GetSpacing()
            ), prediction.SetDirection(t1w_image_n4ss.GetDirection())
    ...
    def process_isles_case(self, input_data, input_filename):
        ...
        origin, spacing, direction = (
            input_data["t1w_image"].GetOrigin(),
        ...
        output_image_path = self._algorithm_output_path / input_filename.replace(
            "T1w", "label-L_mask"
        )
        ...
        if output_image_path.exists():
            json_result = {
                ...
                "inputs": [
                    dict(type="Image", slug="t1w-brain-mri", filename=input_filename)
                ],
            }
