...
class ploras:
    def __init__(
        ...
        if self.debug:
            self._input_path = Path(
                "/home/lchalcroft/mdunet/miccai22_dockers/docker-atlas/test/"
            )
            self._output_path = Path(
                "/home/lchalcroft/mdunet/miccai22_dockers/docker-atlas/output/"
            )
        ...
        self.model_paths = [
            "/opt/algorithm/checkpoints/0/best.ckpt",
            "/opt/algorithm/checkpoints/1/best.ckpt",
            "/opt/algorithm/checkpoints/2/best.ckpt",
            "/opt/algorithm/checkpoints/3/best.ckpt",
        ...
        for i, pth in enumerate(self.model_paths):
            ...
            torch.save(ckpt, pth)
        self.models = [
            NNUnet.load_from_checkpoint(path, map_location=self.device)
    ...
    def predict(self, input_data):
        ...
        img = monai.transforms.NormalizeIntensity(nonzero=True, channel_wise=True)(
            img
        ).astype(np.float32)
        orig_shape = img.shape[1:]
        bbox = monai.transforms.utils.generate_spatial_bounding_box(
            img, channel_indices=-1
        )
        ...
        with torch.no_grad():
            for m in list(self.models):
                pred.append(
                    softmax(m._forward(img).squeeze(0).cpu().detach().numpy(), axis=0)
                )
    ...
    def load_isles_case(self):
        ...
        t1w_json_path = self.get_file_path(
            slug="t1w-mri-acquisition-parameters", filetype="json"
        )

        input_data = {
            "t1w_image": SimpleITK.ReadImage(str(t1w_image_path)),
